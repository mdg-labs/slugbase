...
case "$APP" in
  ...
  marketing)
    ...
    if [[ -n "${PUBLIC_PLAN_PRICE_PERSONAL_MONTHLY:-}" ]]; then
      MARKETING_BUILD_ENV+=(PUBLIC_PLAN_PRICE_PERSONAL_MONTHLY="${PUBLIC_PLAN_PRICE_PERSONAL_MONTHLY}")
    fi
    if [[ -n "${PUBLIC_PLAN_PRICE_PERSONAL_YEARLY:-}" ]]; then
      MARKETING_BUILD_ENV+=(PUBLIC_PLAN_PRICE_PERSONAL_YEARLY="${PUBLIC_PLAN_PRICE_PERSONAL_YEARLY}")
    fi
    if [[ -n "${PUBLIC_PLAN_PRICE_TEAM_SEAT:-}" ]]; then
      MARKETING_BUILD_ENV+=(PUBLIC_PLAN_PRICE_TEAM_SEAT="${PUBLIC_PLAN_PRICE_TEAM_SEAT}")
    fi
    if [[ -n "${PUBLIC_PLAN_PRICE_TEAM_SEAT_YEARLY:-}" ]]; then
      MARKETING_BUILD_ENV+=(PUBLIC_PLAN_PRICE_TEAM_SEAT_YEARLY="${PUBLIC_PLAN_PRICE_TEAM_SEAT_YEARLY}")
    fi
    if [[ -n "${PUBLIC_PLAN_PRICE_SUPPORTER:-}" ]]; then
      MARKETING_BUILD_ENV+=(PUBLIC_PLAN_PRICE_SUPPORTER="${PUBLIC_PLAN_PRICE_SUPPORTER}")
    fi
