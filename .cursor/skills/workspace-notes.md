...
`e2e.yml` mirrors `scripts/e2e.sh`: API/container start with explicit test env only (`SLUGBASE_E2E_MODE`, hardcoded session/encryption keys, localhost URLs, `PUBLIC_REGISTRATION=true`). Infisical `staging` fetch runs **immediately before Playwright** so `REPORTPORTAL_*` reaches the reporter without bleeding `EMAIL_VERIFICATION_REQUIRED` etc. into the API. Follow-up: selective 3-key fetch via Infisical path `/ci/e2e` (option B).
_added: 2026-06-13_
