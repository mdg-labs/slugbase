...
describe("readEditionAwareViteBoolean", () => {
  ...
  it("derives ce defaults when VITE flags are unset", () => {
    ...
    expect(readEditionAwareViteBoolean("VITE_MAIL_ADMIN_UI")).toBe(true);
    expect(readEditionAwareViteBoolean("VITE_OIDC_ADMIN_UI")).toBe(true);
    expect(readEditionAwareViteBoolean("VITE_AI_BYO_CREDENTIAL")).toBe(true);
...
describe("loadWorkspaceInterfaceConfig", () => {
  ...
  it("loads ce workspace admin panels when edition presets apply", async () => {
    vi.stubEnv("SLUGBASE_EDITION", "ce");
