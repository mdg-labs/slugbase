 ...
 *     private readonly wsDataGuard: WorkspaceDataGuard,
 *   ) {}
 *
 *   async getBookmark(activeWorkspaceId: string, bookmarkId: string) {
 *     const bookmark = await this.repo.findById(activeWorkspaceId, bookmarkId);
 *     return this.wsDataGuard.verifyOwnership(activeWorkspaceId, bookmark);
 *   }
 * }
 * ```
 */
