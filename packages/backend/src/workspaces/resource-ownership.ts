 ...
 * @example
 * ```typescript
 * // Inside a future BookmarksService method:
 * const bookmark = await this.repo.findById(activeWorkspace.id, bookmarkId);
 * assertResourceOwnership(activeWorkspace.id, bookmark.workspaceId);
 * await this.repo.delete(activeWorkspace.id, bookmarkId);
 * ```
 */
