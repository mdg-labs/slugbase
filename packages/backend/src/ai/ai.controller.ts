 ...
 *   3. Workspace toggle   - per-workspace enable flag must be on.
 *   4. Provider check     - OPENAI_API_KEY env must be configured.
 */
@Controller("ai")
...
export class AiController {
  ...
  async suggest(
    ...
    if (!(await this.aiSettings.isWorkspaceEnabled(workspace.id))) {
