...
function App() {
  ...
  const [plan, setPlan]       = useState(SB.plan);
  ...
  const [bmModal, setBmModal] = useState(null); // null | { mode:"create" } | { mode:"edit", bookmark }
  ...
  const bulkAct = (kind) => {
    const n = selCount;
    if (kind === "delete") {
      const ids = selAll ? new Set(filtered.map(b => b.id)) : sel;
      setBookmarks(bs => bs.filter(b => !ids.has(b.id)));
      setPlan(p => ({ ...p, used: Math.max(0, p.used - n) }));
      toast({ icon: "trash-2", title: `Deleted ${n} bookmark${n > 1 ? "s" : ""}` });
    } else if (kind === "pin") {
      const ids = selAll ? new Set(filtered.map(b => b.id)) : sel;
      setBookmarks(bs => bs.map(b => ids.has(b.id) ? { ...b, pinned: true } : b));
      toast({ icon: "pin", title: `Pinned ${n} bookmark${n > 1 ? "s" : ""}` });
    } else {
      const verb = { move: "Moved", tag: "Tagged", share: "Shared" }[kind];
      toast({ icon: kind === "move" ? "folder-input" : kind === "tag" ? "hash" : "share-2", title: `${verb} ${n} bookmark${n > 1 ? "s" : ""}` });
    }
    clearSel();
  };
  ...
  const newBookmark = () => {
    ...
    setBmModal({ mode: "create" });
  };

  const handleBmSave = (data) => {
    if (bmModal.mode === "create") {
      setBookmarks(bs => [data, ...bs]);
      setPlan(p => ({ ...p, used: p.used + 1 }));
      toast({ icon: "bookmark-plus", title: "Bookmark saved", sub: data.title });
    } else {
      setBookmarks(bs => bs.map(b => b.id === data.id ? data : b));
      toast({ icon: "check", title: "Changes saved", sub: data.title });
    }
    setBmModal(null);
  };

  const handleBmDelete = (id) => {
    const b = bookmarks.find(x => x.id === id);
    setBookmarks(bs => bs.filter(x => x.id !== id));
    setPlan(p => ({ ...p, used: Math.max(0, p.used - 1) }));
    setBmModal(null);
    toast({ icon: "trash-2", title: "Bookmark deleted", sub: b?.title });
  };
  ...
  const cardOf = (b) => (
    <BookmarkCard key={b.id} b={b} selected={selAll || sel.has(b.id)}
      ...
      onEdit={() => setBmModal({ mode: "edit", bookmark: b })} />
  );

  return (
    <div className="app">
      <Sidebar ws={ws} onSwitch={setWs} nav={nav} setNav={setNav}
      ...
      {bmModal && (
        <BookmarkModal
          mode={bmModal.mode}
          bookmark={bmModal.bookmark || null}
          onSave={handleBmSave}
          onDelete={handleBmDelete}
