...
function TopBar({ ws, count, onOpenPalette, theme, onToggleTheme, onNew, crumbIcon = "bookmark", crumbLabel = "Bookmarks", crumbSub = "All", archivedCount = 0 }) {
  ...
  return (
    <header className="topbar">
      <div className="crumbs">
        <Icon name={crumbIcon} size={15} style={{ color: "var(--accent-text)" }} />
        <b>{crumbLabel}</b>
        {crumbSub && <><Icon name="chevron-right" size={14} /><span>{crumbSub}</span></>}
        {count != null && <span className="badge badge--neutral" style={{ marginLeft: 2 }}>{count}</span>}
      </div>
      ...
      <div className="topbar-right">
        {archivedCount > 0 && (
          <div className="topbar-archived"><Icon name="archive" size={13}/>{archivedCount} archived</div>
        )}
