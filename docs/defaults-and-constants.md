...
**Operator follow-up:** staging smoke (`.github/scripts/smoke-staging-health.sh`) hits `FRONTEND_ORIGIN` and `MARKETING_ORIGIN` — requires a successful staging deploy after merge. First deploy creates Worker scripts and custom domains when the `slugbase.app` zone is on the deploy Cloudflare account and the API token has Workers + DNS permissions.
