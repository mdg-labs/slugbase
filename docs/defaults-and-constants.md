...
| `FRONTEND_ORIGIN` | Public web app URL (Workers custom domain) | `staging` / `production` | Staging: `https://staging-cloud.slugbase.app`; production: `https://cloud.slugbase.app` |
| `MARKETING_ORIGIN` | Public marketing site URL (Workers custom domain) | `staging` / `production` | Staging: `https://staging.slugbase.app`; production: `https://slugbase.app` |
...
| `CF_ACCESS_CLIENT_ID` | Cloudflare Access service token id for CI smoke (`CF-Access-Client-Id` header) | `staging` | Set in Infisical; omitted in production (public hostnames) |
| `CF_ACCESS_CLIENT_SECRET` | Cloudflare Access service token secret for CI smoke | `staging` | Pair with `CF_ACCESS_CLIENT_ID`; CI only — never client bundle |
...
**Operator follow-up:** staging smoke (`.github/scripts/smoke-staging-health.sh`) probes `APP_BASE_URL` and `FRONTEND_ORIGIN` via `/health` and `/version`; marketing liveness is `GET ${MARKETING_ORIGIN}/` (site root, HTTP 200) — not `/health` or `/version` on the static marketing Worker. Requires a successful staging deploy after merge. Staging hostnames sit behind Cloudflare Access; set `CF_ACCESS_CLIENT_ID` and `CF_ACCESS_CLIENT_SECRET` in Infisical `staging` so smoke sends service-token headers. First deploy creates Worker scripts and custom domains when the `slugbase.app` zone is on the deploy Cloudflare account and the API token has Workers + DNS permissions.
