...
If you sign in via **SSO (OIDC)**, MFA is handled by your identity provider — SlugBase does not add an extra code step after SSO login. See [MFA and SSO (OIDC)](#mfa-and-sso-oidc) below.

<Callout kind="tip">
  Your deployment can set **`MFA_ISSUER_NAME`** so the account label in your authenticator app matches your instance (for example your workspace name). If it is unset, entries typically show **SlugBase**.
</Callout>
...
- **OIDC-only accounts** (no local password): SlugBase does **not** show a second TOTP step after SSO. Your identity provider is expected to enforce MFA according to its own policy.
