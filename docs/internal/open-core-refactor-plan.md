...
- [ ] Implement §14.11
- [ ] Export path from `@slugbase/backend`
...
- [ ] Implement `SlugbaseMollieBillingService` using commerce packages
- [ ] Port stash controller routes (cancel, reactivate, change-plan, payment-method, mollie webhook)
- [ ] Delete portal flow; update web `billing-api.ts` if needed (already in slugbase CE web)
- **Verify:** `pnpm --filter @slugbase/slugbase-billing test:unit`
...
| TASK-012 | `[ ]` | slugbase | 011 | | `packages/backend/src/billing/**`, stripe deletion, `domain-modules.ts` |
